...
resolveChallenge c st =
  case (challengeStatus c, st) of
    (ChallengeOpen, ChallengeUpheld) ->
      Right c { challengeStatus = ChallengeUpheld }
    (ChallengeOpen, ChallengeRejected) ->
      Right c { challengeStatus = ChallengeRejected }
...
finalize p cs now
  | canFinalize p cs now = Right p { proposalStatus = Canonical
                                   , finalizationAt = Just now }
