...
## 2026-09-25 — Autonomous cycle: P2.8 exact-head failure classified and dependency repair

**Exact failing run:** `36187719234` on HEAD `bc519996ee2447578543376124d7a51002e117e7`.

### P2.8 evidence captured
- Job `ledger-runner` completed **FAILURE**.
- Checkout and GHC/Cabal installation completed successfully.
- Failure occurred before `--evaluate` could execute: Cabal dependency solving stopped on **unknown package `cardano-prelude`**, required by `cardano-crypto-wrapper-1.7.0.0`.
- The uploaded evidence artifact `p2-8-b1-cardano-ledger-runner-evidence` contains `runner.log`, confirming this is a **runner dependency/infrastructure failure**, not a Plutus validator failure and not evidence about the Reveal transaction.
- Therefore P2.8 remains **EVIDENCE GAP / INFRASTRUCTURE BLOCKER**; no evaluator result is claimed.

### Root-cause triangulation
The current `audit/cardano-ledger-runner/cabal.project` pinned `cardano-prelude-test` from the required upstream revision but omitted the sibling `cardano-prelude` package. The pinned upstream `cardano-crypto-wrapper-1.7.0.0` explicitly requires `cardano-prelude >=0.2.0.0`. The pinned `cardano-prelude` revision contains the `cardano-prelude` package at version `0.2.0.0`, so the repair is to expose that package in the same pinned source repository rather than changing ledger versions or weakening the evaluator.

### Repair committed
Commit `ecfe741386366d25177779d916fe95c1d38a0bd9` updates `audit/cardano-ledger-runner/cabal.project` to include:
- `cardano-prelude`
- `cardano-prelude-test`
from the same pinned upstream revision `cfb3e0784f0d483f4d5c4f047f324b7dc8091475`.

No evaluator semantics, protocol economics, validator logic, or Cardano size limits were changed.

### Kernel
Run `36187719357` on the prior HEAD was cancelled before the regression suite. No Kernel conclusion is promoted.

### Other fronts
Materios/B3, GOV-28, fee boundary, B4/B5/B6, and universal/application boundary remain as previously classified. The P2.8 repair is isolated to dependency materialization and does not alter those semantics.

**Next autonomous action:** observe the new exact-head P2.8 run generated by `ecfe741...`; classify only its actual dependency/build/evaluator result and artifact.

**USER QUESTION:** NONE.
## 2026-09-25 — External static audit triangulation

An external static/documentary audit dated 2026-09-25 was reviewed against the current branch. The audit explicitly states that it was a repository/static review, did not execute tests, did not compile Plutus/Haskell, and did not verify a live deployment. [external-audit: lines 35-46]

### Findings confirmed by current evidence

- Independent Plutus security review remains an outstanding prerequisite before real-user funds. The external audit itself identifies this as the highest-priority missing assurance. [external-audit: lines 179-188]
- Bus factor / independent human review remains an operational risk. The audit reports one repository committer identity dominating the observed history and no evidence of independent peer review on the analysed branch. This is process evidence, not a code-correctness finding. [external-audit: lines 77-90]
- The audit's distinction between mathematical verification and implementation conformance is aligned with the repository's own VERIFICATION_STATUS.md: the mathematical baseline is separate from implementation evidence, and the implementation is not claimed to be formally verified. [external-audit: lines 152-160]
- B3 remains an evidence target rather than a closed deployment property. The audit's description of B1 versus the B3 target is consistent with the current coordination state. [external-audit: lines 115-123]

### Findings that are snapshot-sensitive or require qualification

1. "No tests executed" describes the external audit methodology, not the current project state. Since that snapshot, the branch has accumulated CI evidence including Cardano adapter conformance, Algorithmic Governability and Materios/B3 test workflows. Those are evidence artifacts, not substitutes for an independent audit.
2. "No formal mechanized proof" needs precise wording. The repository contains a mathematical proof corpus plus a finite-model mechanical checker, but no proof-assistant artifact establishing implementation conformance. The audit itself makes this distinction explicitly. [external-audit: lines 152-160]
3. Qualitative statements such as "solid" or "above average" are external evaluator judgements, not protocol facts. Keep them attributed and do not promote them to project evidence.
4. Commit-rate / AI-agent conclusions are observational hypotheses about development process. They must not be treated as proof that code is correct or incorrect, and the identity of the coding agent cannot be inferred with certainty from cadence alone.
5. Test-count figures such as 544 tests / 48 files are dated snapshot counts. Current CI evidence is authoritative for present coverage/status.
6. The audit's RTP discussion concerns the PRE-RICH application profile and must not be confused with IMMORTAL universal economics.

### New assurance front

Create and maintain a distinct INDEPENDENT-ASSURANCE front:

- independent Plutus/UPLC security review;
- second-human review of economic and authority-critical paths;
- review of MintPolicy, B1PrizePool, PrizeValidator, CounterValidator, BeaconRegistry and reachable Treasury surfaces;
- explicit review of B3/Materios proof boundaries;
- review of economic admission, executable liquidity, oracle/EEV and fee settlement;
- reproducible build and artifact identity review;
- final deployment readiness review.

This is external assurance / release readiness, not a reason to alter frozen economics or rewrite validators merely to satisfy an audit narrative.

### Immediate autonomous consequence

Until independent review exists, do not label the system production-ready or imply that CI/conformance replaces a specialist smart-contract security audit. Continue closing technical evidence in parallel; preserve the audit's useful criticism without turning its qualitative rankings into project facts.

**Status:** EXTERNAL AUDIT TRIANGULATED / INDEPENDENT ASSURANCE OPEN / NO NORMATIVE ECONOMIC CHANGE.

## 2026-09-25 — Autonomous cycle: P2.8 second dependency repair

**Current HEAD:** `b0faf0b2fe1c2aa3f80676aa5efa6961e21c5b3e` on `work/immortal-green-closure`.

### P2.8 exact evidence
Run `36188428430` on repair commit `ecfe741386366d25177779d916fe95c1d38a0bd9` was observed directly. The first dependency failure (`cardano-prelude`) is resolved. The runner now reaches dependency resolution and fails on the next omitted pinned package: **`cardano-crypto-class`**, required by `cardano-ledger-alonzo-1.16.0.0`.

The pinned `cardano-base` revision `a02ed49194501620f03b2f868aabd5a48ebde937` was independently inspected and contains `cardano-crypto-class`. Therefore the minimal source-grounded repair is to expose that sibling package in the same pinned source repository.

### Repair committed
Commit `b0faf0b2fe1c2aa3f80676aa5efa6961e21c5b3e` adds `cardano-crypto-class` to the existing pinned `cardano-base` source package list.

No ledger version, evaluator semantics, validator, economic rule, transaction-size limit or protocol constant was changed.

### Other fronts
No normative boundary was reached. Fee semantics remain parameter-open; Materios/B3 remains evidence-open; GOV-28 provenance remains evidence-open; B4/B5/B6 remain conformance/evidence fronts; independent assurance remains open.

### Classification
- P2.8: **IMPLEMENTATION / INFRASTRUCTURE GAP**, iteratively closing dependency materialization before evaluator execution.
- Economic semantics: **UNCHANGED**.
- User decision boundary: **NONE**.

### Next autonomous action
Observe the workflow generated by `b0faf0b...` and continue repairing only source-grounded dependency gaps until the native evaluator actually executes; then capture the exact evaluator result/artifact.

**USER QUESTION:** NONE.


## 2026-09-25 — Autonomous cycle: evidence-first sweep / fee boundary recheck

**Current branch:** `work/immortal-green-closure`  
**HEAD observed in this coordination file:** `b0faf0b2fe1c2aa3f80676aa5efa6961e21c5b3e`

### P2.8 / CI

The GitHub connector exposes **no PR workflow run for this exact HEAD**. The previously observed runner repair remains evidence-only until a run against the current commit produces the native evaluator artifact. No historical run is promoted to current evidence. Local network access from this session cannot substitute for CI evidence.

**Classification:** P2.8 = **EVIDENCE GAP / IMPLEMENTATION-INFRASTRUCTURE GAP**.  
**No semantic change made.**

### Protocol Usage Fee / Settlement boundary

A fresh Notion search reconfirmed the existing workflow family, including:

- `IMMORTAL — Economic Usage Fee & Adapter Settlement Workflow v0.1`;
- `12 — Reference Unit, Target Function & Fee Affordability Formalization v0.1`;
- `23 — DApp → Cardano Adapter Operational Bridge & Live Conformance v0.1`.

The available source material continues to support the existing separation: IMMORTAL determines the economic fee obligation/affordability contract; the Adapter materializes valuation, conversion and chain settlement. The unresolved parameter choices are therefore not silently closed by implementation observations.
...
**Classification:** Protocol Usage Fee = **OPEN / RESEARCH**; settlement realization = **IMPLEMENTATION / CONFORMANCE FRONT**.  
**No normative fee value or PRE-RICH-specific universal rule introduced.**

### Governance / GOV-28

Current working-branch replay code still carries non-empty canonicalization references, but the reference strings alone do not establish cryptographic identity between the DecisionRecord and CanonicalizationRecord. Creating such a binding would require a defined canonical preimage/identity rule, which is not derivable from the current evidence. Therefore no new hash-binding semantics were invented.

**Classification:** Governance provenance binding = **EVIDENCE GAP / OPEN DESIGN QUESTION**, not a closure.

### Environment limitation

A direct local `git clone` was attempted for independent test execution but DNS/network access to GitHub was unavailable in the runtime. This is recorded only as an environment limitation; it does not alter protocol status.

### Handoff

- **Fronts advanced:** P2.8 evidence state; Protocol Usage Fee / Settlement boundary; Governance provenance.
- **Concrete repo change:** this coordination handoff only.
- **Tests/CI observed:** no new current-HEAD workflow evidence; no local test result claimed.
...
## 2026-09-25 — Autonomous cycle: Materios commitment-boundary hardening

**Commit:** `352b4b2cb649685fa5fd26d8df2637f2fe21e99d`

### Materios / Beacon 3
Added integration coverage for the local, cryptographically checkable boundary that binds recovered Materios selection-input bytes to the on-chain `selection_inputs_hash` commitment.

The new tests establish:
- exact selection-input bytes hash to the supplied 32-byte on-chain commitment;
- tampered selection-input bytes fail closed with `SELECTION_INPUTS_ONCHAIN_COMMITMENT_MISMATCH`;
- transition-level commitment binding requires the exact activation block number as well as the exact block hash;
- an activation-block number mutation is rejected with `SELECTION_INPUTS_COMMITMENT_BLOCK_NUMBER_MISMATCH`.

This is deliberately **not** a Materios selector implementation and does not claim the missing runtime proof verifier. It hardens only the boundary that can be checked locally once canonical block data and selection-input bytes are supplied.

### P2.8
The branch remains wired to the native `--evaluate` runner. No current-head workflow run is exposed by the connector yet. The new Materios test commit also pushes the branch and therefore should trigger the existing P2.8 push workflow; until a run/artifact is observed, no evaluator result is promoted.

### Other fronts
- GOV-28: no new canonicalization identity rule invented.
- B4/B5/B6: no semantic rule changed; executable-liquidity/protected-capital evidence remains open.
- Protocol Usage Fee: no numerical parameter selected; IMMORTAL/Adapter ownership boundary unchanged.
- Independent assurance: remains open and separate from CI evidence.

**USER QUESTION:** NONE.

**Next autonomous action:** observe the exact-head CI generated by `352b4b2...`; in parallel continue source-grounded Materios finality/proof composition and other evidence fronts without changing frozen economics.


## 2026-09-25 — Autonomous cycle: Action Refinement conformance mismatch repaired

**Code commit observed:** `608a8cf965760a65b4e18399048c44eabe250cef`.

### Exact failure

PRE-RICH Action Refinement workflow run `36189642877` checked out the exact code commit and executed 76 tests: **75 passed / 1 failed**. The only failure was `canonical-transition-evidence.test.ts`: the test expected the legacy error `preStateFingerprint must be non-empty`, while the current validator correctly rejected the empty value earlier with the stricter digest-contract error `preStateFingerprint must be a 32-byte hex digest`.

This is a **test expectation / validation-contract mismatch**, not an economic or validator failure.

### Repair

Commit `2369de040a2e024627843719615e42c56c155f82` updates the three empty-fingerprint assertions to expect the current 32-byte hex-digest validation contract. No runtime semantics were changed.

### Classification

- PRE-RICH Action Refinement: **IMPLEMENTATION/TEST CONFORMANCE GAP → REPAIRED; awaiting fresh CI evidence**.
- P2.8: still **IN PROGRESS / EVIDENCE GAP** on its native runner.
- Kernel Invalid-Class: exact-head run remains **IN PROGRESS** at the last observation.
- B3/Materios, GOV-28, B4/B5/B6, fee parameters and independent assurance remain as previously classified.

**USER QUESTION:** NONE.

**Next autonomous action:** observe the CI generated by `2369de0...`; promote the Action Refinement result only after exact-head evidence is captured, while continuing the other active fronts.

## 2026-09-25 — Autonomous cycle: GOV-28 provenance gap isolated precisely

### Governance / canonicalization provenance
Direct inspection of the working branch confirms that GOV-28 lifecycle handling is materially separated from the legacy `Governance.hs` state machine:

- `GovernanceCanonicalReplay.applyCanonicalEvent` has dedicated handlers for `DECISION_FINALIZED`, `ADOPTION_RECORDED`, `CONFORMANCE_RECORDED`, and `CANONICALIZED`.
- Legacy collapsed `Accepted → Adopted → Canonical` status events are rejected at the canonical replay boundary.
- `CanonicalizationRecord` already carries `canonicalizationDecisionRecordReference`.
- `DecisionRecord` already carries `decisionCanonicalizationReference`.

The remaining gap is narrower than previously stated: `canonicalizationRecordValid` checks that both references are non-empty, but the replay path does **not** currently prove that the canonicalization reference equals the reference established by the finalized DecisionRecord. The canonicalization handler also receives only the immediate predecessor event, while the canonical governance state does not persist the full DecisionRecord witness.

### Classification
**GOV-28 = IMPLEMENTATION/PROVENANCE GAP, precisely localized.**

No new hash/preimage/reference semantics were invented. A fix requires a source-grounded decision on how the finalized DecisionRecord witness is carried into canonical replay (or an equivalent canonical-history lookup) before the reference equality can be enforced safely.

### Materios / Beacon 3
The working branch already contains the exact selection-input/on-chain commitment tests in `authority-transition.test.ts`:
- recovered input bytes must hash to the supplied on-chain commitment;
- commitment hash mutation fails closed;
- activation block hash/number must match the finality checkpoint;
- authority transition proof remains explicitly unresolved/structural in tests.

Therefore the current remaining B3 work is **not another local hash-binding test**; it is obtaining/decoding the real canonical finalized block commitment, matching the exact runtime input bytes, and composing the verified transition with actual GRANDPA finality and runtime/storage proof evidence.

### P2.8 / CI
The available GitHub workflow-run connector still exposes no runs for the recent repair commits `2369de0...`, `608a8cf...`, or `5ce9629c...`. Consequently no current-head P2.8 evaluator or Action Refinement result is promoted from connector absence.

GitHub's workflow syntax confirms that the configured `push.branches` filter is sufficient to trigger the workflow for pushes to the named working branch. citeturn0search0turn0search1

### Other fronts
- B4/B5/B6: remain evidence/conformance fronts; no new universal economic rule introduced.
- Protocol Usage Fee: remains open/research; no numerical parameter selected.
- Universal/Application boundary: conceptual split remains supported; destructive refactor still deferred until consumer mapping is complete.
- Research-novelty: prior-art baseline remains open; no novelty claim.
- Independent assurance: remains open.
