...
main = do
  ...
  assert "ordinary 50/50 rejected"
    (not (approvalReached DocumentationOnly
      (Snapshot 2 0 [(1,50),(2,50)]) []
  ...
  assert "recursive chain rejected"
    (not (delegationValid
      (Snapshot 2 0 [(1,30),(2,30),(3,40)])
      [Delegation 1 2, Delegation 2 3]))

  -- Lifecycle / replay
  s1 <- expectRight "proposal submission" $
    applyEvent emptyState (ProposalSubmitted baseProposal)

  s2 <- expectRight "proposal replay" $
    replay s1
  ...
  let p = head (proposals s2)

  assert "first eligible vote accepted"
    (case applyEvent s2
      (VoteCast (Vote 1 1 For (4 + communityReviewSeconds))) of
        Right _ -> True
        Left _ -> False)

  s3 <- expectRight "vote application" $
    applyEvent s2
      (VoteCast (Vote 1 1 For (4 + communityReviewSeconds)))

  assert "duplicate vote rejected"
    (case applyEvent s3
      (VoteCast (Vote 1 1 Against
        (4 + communityReviewSeconds + 1))) of
        Left _ -> True
        Right _ -> False)
  ...
  assert "ineligible voter rejected"
    (case applyEvent s2
      (VoteCast (Vote 1 99 For
        (4 + communityReviewSeconds))) of
        Left _ -> True
        Right _ -> False)

  assert "delegated voter cannot direct vote"
    (case applyEvent s2
      (DelegationSet 1 (Delegation 1 2)
        (4 + communityReviewSeconds)) of
        Left _ -> False
        Right sd ->
          case applyEvent sd
            (VoteCast (Vote 1 1 For
              (4 + communityReviewSeconds + 1))) of
              Left _ -> True
              Right _ -> False)
  ...
  assert "invalid classification rejected"
    (case applyEvent
      (emptyState {proposals=[baseProposal]})
